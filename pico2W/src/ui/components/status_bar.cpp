...
void StatusBar::render(const StatusSnapshot& status) const {
    ...
    std::snprintf(line_top, sizeof(line_top), "M:%s SD:%s T:%s %s", status.machine, status.sd, status.tool, status.time_text);

    const int16_t text_y = static_cast<int16_t>((UiLayout::kTopBarHeight - display_.text_height(2)) / 2);
    display_.draw_text(UiLayout::kScreenMarginX, text_y, line_top, COLOR_TEXT, COLOR_HEADER, 2);
}
