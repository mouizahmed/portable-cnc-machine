...
void SelectableListRow::render(const SelectableListRowSpec& spec) const {
    ...
    display_.draw_text(static_cast<int16_t>(spec.rect.x + 8), static_cast<int16_t>(spec.rect.y + 7), spec.primary_text, COLOR_TEXT, fill, 1);
    display_.draw_text(static_cast<int16_t>(spec.rect.x + 118), static_cast<int16_t>(spec.rect.y + 7), spec.secondary_text, detail_color, fill, 1);
}
