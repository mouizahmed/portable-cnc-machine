...
{
    private const double ContinuousJogDistanceMm = 1000.0;
    ...
    public ICommand JogPadCommand          { get; }
    ...
    public ManualControlViewModel()
    {
        ...
        JogPadCommand          = new RelayCommand<string>(JogPadClicked);
    ...
    public void JogPadPointerPressed(string? action)
    {
        if (!ContinuousJog || !CanJogControls || string.IsNullOrWhiteSpace(action))
            return;

        if (!TryParseJogAction(action, out var x, out var y, out var z))
            return;

        var activeAxes = (x != 0 ? 1 : 0) + (y != 0 ? 1 : 0) + (z != 0 ? 1 : 0);
        if (activeAxes != 1)
        {
            SetCommandPreview("Continuous diagonal jog is not supported yet");
            if (MainVm != null)
            {
                MainVm.StatusMessage = "Use single-axis buttons for continuous jog.";
        ...
        if (x != 0)
            Jog('X', Math.Sign(x) * ContinuousJogDistanceMm);
        else if (y != 0)
            Jog('Y', Math.Sign(y) * ContinuousJogDistanceMm);
        else
            Jog('Z', Math.Sign(z) * ContinuousJogDistanceMm);

        _continuousJogActive = true;
    }

    public void JogPadPointerReleased(string? action)
    {
        ...
        if (ContinuousJog)
        {
            CancelContinuousJogIfActive();
            return;
        }

        JogPadClicked(action);
    }

    private void JogPadClicked(string? action)
    {
        if (ContinuousJog || string.IsNullOrWhiteSpace(action))
            return;
        ...
        if (z != 0)
            Jog('Z', Math.Sign(z) * JogStep);
        else if (x != 0 && y != 0)
            JogXY(Math.Sign(x) * JogStep, Math.Sign(y) * JogStep);
        else if (x != 0)
            Jog('X', Math.Sign(x) * JogStep);
        else if (y != 0)
            Jog('Y', Math.Sign(y) * JogStep);
    }

    public void JogPadPointerCaptureLost()
    {
        CancelContinuousJogIfActive();
    }

    private void CancelContinuousJogIfActive()
    {
        if (!_continuousJogActive)
            return;
    ...
    private static bool TryParseJogAction(string action, out int x, out int y, out int z)
    {
        x = 0;
        y = 0;
        z = 0;

        foreach (var token in action.Split(' ', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
        {
            if (token.Length != 2)
                return false;

            var sign = token[1] switch
            {
                '+' => 1,
                '-' => -1,
                _ => 0
