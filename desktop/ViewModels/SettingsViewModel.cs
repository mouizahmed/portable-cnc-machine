...
public sealed class SettingsViewModel : PageViewModelBase
{
    ...
    public bool IsControllerConnected => MainVm?.ControllerConnectionStatus == ConnectionStatus.Connected;
    ...
    {
        if (propertyName != nameof(MainWindowViewModel.ControllerConnectionStatus))
            return;
    ...
    public void ApplyMachineSettings(ControllerMachineSettings settings)
    {
    ...
    private async void SaveLocalSettings()
    {
        ...
        try
        {
            ...
            {
                if (!HasMachineSettingsLoaded || !IsControllerConnected)
                {
                    ClearStatusOverride();
                    UpdateDerivedState();
                    MainVm.StatusMessage = hadPendingLocalChanges
                        ? "App settings saved. Machine settings are still pending."
                        : "Machine settings are pending. Connect to the controller to save them.";
    ...
    private void UpdateDerivedState()
    {
        ...
        RaisePropertyChanged(nameof(IsControllerConnected));
    ...
    private void RefreshAvailablePorts()
    {
        ...
        try
        {
            foreach (var port in UsbDeviceService.GetControllerPorts())
            {
    ...
    private string BuildStatusDetail()
        => _machineSettingsLoadState switch
        {
            MachineSettingsLoadState.Loading => "Requesting the current machine settings snapshot from the controller.",
            MachineSettingsLoadState.Error => _machineSettingsLoadError ?? "Unable to load machine settings from the controller.",
            MachineSettingsLoadState.Unavailable when !IsControllerConnected => "Machine sections stay hidden until the controller is connected and settings load succeeds.",
