...
public sealed class MainWindowViewModel : ViewModelBase, IDisposable
{
    ...
    public SerialService GrblSerial { get; } = new();
    ...
    /// </summary>
    public ControllerProtocolService Protocol { get; }
    ...
    private ConnectionStatus _controllerConnectionStatus = ConnectionStatus.Disconnected;
    /// <summary>Connection status for the desktop → Controller USB CDC link.</summary>
    public ConnectionStatus ControllerConnectionStatus
    {
        get => _controllerConnectionStatus;
        set
        {
            if (SetProperty(ref _controllerConnectionStatus, value))
            {
    ...
    public bool IsConnected => ControllerConnectionStatus == ConnectionStatus.Connected;
    ...
    public string ConnectionStatusText
    {
        get
        {
            ...
            if (ControllerConnectionStatus == ConnectionStatus.Connecting)
                return "CONNECTING...";
            return "DISCONNECTED";
        }
    }
    ...
    // ════════════════════════════════════════════════════════════════
    // MACHINE OPERATION STATE  (driven by STATE from the controller)
    // ════════════════════════════════════════════════════════════════

    private MachineOperationState _machineState;
    /// <summary>
    /// Unified 13-state FSM. Set exclusively by ControllerProtocolService.StateChanged.
    ...
    {
        _ when ControllerConnectionStatus != ConnectionStatus.Connected => "DISCONNECTED",
    ...
    {
        _ when ControllerConnectionStatus != ConnectionStatus.Connected => ThemeResources.Brush("NeutralStateBrush", "#808080"),
    ...
    // ════════════════════════════════════════════════════════════════
    // SAFETY LEVEL  (driven by SAFETY from the controller)
    // ════════════════════════════════════════════════════════════════

    private SafetyLevel _safetyLevel;
    /// <summary>Orthogonal safety supervision level. Set by ControllerProtocolService.SafetyChanged.</summary>
    ...
    {
        _ when ControllerConnectionStatus != ConnectionStatus.Connected => "--",
    ...
    // ════════════════════════════════════════════════════════════════
    // CAPABILITY FLAGS  (driven by CAPS from the controller)
    // ════════════════════════════════════════════════════════════════

    private CapsFlags _caps;
    /// <summary>Per-action capability flags. Set by ControllerProtocolService.CapsChanged.</summary>
    ...
    // ════════════════════════════════════════════════════════════════
    // MACHINE / WORK COORDINATES  (driven by POS from the controller)
    // ════════════════════════════════════════════════════════════════
    ...
    public string SpindleStatusText => ControllerConnectionStatus != ConnectionStatus.Connected
        ? "DISCONNECTED"
    ...
    public string FeedSpindleSummary => ControllerConnectionStatus != ConnectionStatus.Connected
        ? "-- / --"
    ...
    private JobRunState _jobState = JobRunState.NoJob;
    public JobRunState JobState
    {
        get => _jobState;
        private set
        {
            ...
            {
                RaisePropertyChanged(nameof(JobStateLabel));
                RaisePropertyChanged(nameof(JobStateBrush));
                RaisePropertyChanged(nameof(CanStart));
                RaisePropertyChanged(nameof(CanPause));
                RaisePropertyChanged(nameof(CanStop));
            }
    ...
    public string JobStateLabel => JobState switch
    {
        JobRunState.NoJob    => "NO JOB",
        JobRunState.Loaded   => "LOADED",
        JobRunState.Running  => "RUNNING",
        JobRunState.Hold     => "HOLD",
        JobRunState.Complete => "DONE",
        JobRunState.Failed   => "FAILED",
        JobRunState.Aborted  => "ABORTED",
        _                    => "UNKNOWN"
    };

    public IBrush JobStateBrush => JobState switch
    {
        JobRunState.Loaded   => ThemeResources.Brush("SuccessBrush",      "#3BB273"),
        JobRunState.Running  => ThemeResources.Brush("InfoBrush",         "#5B9BD5"),
        JobRunState.Hold     => ThemeResources.Brush("WarningBrush",      "#E0A100"),
        JobRunState.Complete => ThemeResources.Brush("SuccessBrush",      "#3BB273"),
        JobRunState.Failed   => ThemeResources.Brush("DangerBrush",       "#D83B3B"),
        JobRunState.Aborted  => ThemeResources.Brush("NeutralStateBrush", "#808080"),
        _                    => ThemeResources.Brush("NeutralStateBrush", "#808080")
    };
    ...
    public int CurrentLine
    {
        ...
        private set
        {
            if (SetProperty(ref _currentLine, value))
                RaisePropertyChanged(nameof(JobProgressLineText));
        }
    }
    ...
    public string JobProgressLineText
    {
        get
        {
            if (TotalLines > 0)
                return $"Line {CurrentLine} of {TotalLines}";
            return CurrentLine > 0 ? $"Line {CurrentLine}" : "Line --";
        }
    }
    ...
    public bool CanStart => ControllerConnectionStatus == ConnectionStatus.Connected &&
                            (Caps.JobStart || Caps.JobResume) &&
                            CurrentFileName != null &&
                            JobState is JobRunState.Loaded or JobRunState.Hold or JobRunState.Complete or JobRunState.Aborted;

    public bool CanPause => ControllerConnectionStatus == ConnectionStatus.Connected &&
                            Caps.JobPause &&
                            JobState == JobRunState.Running;

    public bool CanStop  => ControllerConnectionStatus == ConnectionStatus.Connected &&
                            Caps.JobAbort &&
                            JobState is JobRunState.Running or JobRunState.Hold;

    public bool CanHome  => ControllerConnectionStatus == ConnectionStatus.Connected && Caps.Motion;

    public bool CanJog   => ControllerConnectionStatus == ConnectionStatus.Connected && Caps.Motion;
    ...
    public MainWindowViewModel()
    {
        ...
        Protocol = new ControllerProtocolService(Serial);
    ...
    {
        if (Caps.JobResume && JobState == JobRunState.Hold)
        {
            ...
            if (!result.Success && CurrentFileName != null)
                JobState = JobRunState.Hold;
        }
    ...
    private void OnPositionChanged(ControllerPos pos)
    {
        MachineX = pos.MX;
        MachineY = pos.MY;
        MachineZ = pos.MZ;
    ...
    {
        if (ControllerConnectionStatus != ConnectionStatus.Connected)
            return;
    ...
    private void HandleMachineSettingsReceived(ControllerMachineSettings settings)
    {
    ...
    private void HandleProtocolEvent(string name, IReadOnlyDictionary<string, string> kv)
    {
        ...
        switch (name)
        {
            case "JOB_PROGRESS":
                ...
                Progress = TotalLines > 0 ? Math.Clamp(100.0 * CurrentLine / TotalLines, 0, 100) : 0;
                break;

            case "JOB_COMPLETE":
                JobState = JobRunState.Complete;
                Progress = 100;
    ...
    private async Task LoadMachineSettingsOnConnectAsync(int version)
    {
        ...
        var completion = new TaskCompletionSource<ControllerMachineSettings>(TaskCreationOptions.RunContinuationsAsynchronously);

        void HandleMachineSettings(ControllerMachineSettings settings)
        ...
        try
        {
            ...
            if (version != _machineSettingsLoadVersion || ControllerConnectionStatus != ConnectionStatus.Connected)
                return;
