...
public partial class MainWindow : Window
{
    ...
    private void OnDataContextChanged(object? sender, EventArgs e)
    {
        if (_viewModel != null)
        {
            ...
            _viewModel.FilesVm.UnloadLoadedJobRequested -= OnUnloadLoadedJobRequested;
        }
    ...
    private async void OnUnloadLoadedJobRequested(object? sender, string filename)
    {
        if (_viewModel == null)
            return;
        ...
        var dialog = new ConfirmDialog(
            "Unload Job",
            $"Unload '{filename}' from the active job slot?",
            "Unload");

        bool? result = await dialog.ShowDialog<bool?>(this);
        if (result == true) _viewModel.FilesVm.ConfirmUnloadLoadedJob();
        else                _viewModel.FilesVm.CancelUnloadLoadedJob();
    }
}
